...
from projects.titanic.src.config import EVALUATION_METRICS, ML_MODELS
...
class ModelEvaluator:
    ...
    def evaluate_single_model(self, model_name, model_data, detailed=True):
        ...
        if hasattr(model, 'predict_proba'):
            try:
                logger.debug("Calcolo probabilitÃ  predizioni")
                y_pred_proba = model.predict_proba(X_test_processed)[:, 1]
                self.probabilities[model_name] = y_pred_proba
            except Exception as e:
                logger.warning(f"Errore calcolo probabilitÃ  per {model_name}: {str(e)}")
    ...
    def _calculate_probability_metrics(self, y_pred_proba):
        """Calcola metriche basate su probabilitÃ """
...
class InterpretabilityMetrics:
    """
    Metriche per interpretabilitÃ  modelli
    """
    
    @staticmethod
    def calculate_feature_stability(feature_importances_history):
        """
        Calcola stabilitÃ  feature importance nel tempo
        
        Args:
            feature_importances_history: Lista di dizionari feature importance
        
        Returns:
            Score di stabilitÃ  per feature
        """
        logger.info("Calcolo stabilitÃ  feature importance")
        
        if len(feature_importances_history) < 2:
            logger.warning("Dati insufficienti per calcolo stabilitÃ ")
            return {}
        
        # Converti in DataFrame
        df = pd.DataFrame(feature_importances_history)
        logger.debug(f"Analisi stabilitÃ  per {len(df.columns)} features su {len(df)} periodi")
        ...
        logger.debug(f"StabilitÃ  media: {np.mean(list(stability_scores.values()))}")
    ...
    def calculate_prediction_confidence_distribution(probabilities):
        ...
        Args:
            probabilities: Array di probabilitÃ 
        ...
        # Converte probabilitÃ  in confidence (distanza da 0.5)
