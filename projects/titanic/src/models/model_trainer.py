...
from projects.titanic.src.models.ml_models import ModelFactory, ModelEnsemble, HyperparameterGrids
from projects.titanic.src.config import PREPROCESSING_CONFIG, MODELS_DIR
            ...
            if fit:
                ...
                if col in self.label_encoders:
                    ...
                    try:
                        ...
                        # Se ci sono valori non visti, usa il valore piÃ¹ frequente
...
class ModelTrainer:
    ...
    def prepare_data(self, X, y, test_size=0.2, stratify=True):
        ...
        if test_size == 0 or test_size is None:
            logger.info("Nessuno split applicato - usando dati cosÃ¬ come sono")
            self.X_train = X
            self.y_train = y
            self.X_test = None
            self.y_test = None
    ...
    def cross_validate_model(self, model_type, cv_folds=5, scoring='accuracy'):
        ...
        if model_type not in self.trained_models:
            logger.error(f"Model {model_type} non Ã¨ stato addestrato")
            raise ValueError(f"Model {model_type} non Ã¨ stato addestrato")
